function Setting() {
    ...
    (this.checkUpdate = function(version, sw) {
        var sv = parseFloat(version.substr(version.length - 5, version.length))
        var urls = ""
        if (sw == "1") {
            urls = "https://api.github.com/repos/lipic/wattmeter_tst/contents/";
        } else {
            urls = "https://api.github.com/repos/lipic/wattmeter/contents/";
        }
        $.ajax({ url: urls }).done(function(t) {
            for (var i in t) {
                if (t[i].name.includes("tst") || t[i].name.includes("rev")) {
                    sf = parseFloat(t[i].name.substr(t[i].name.length - 5, t[i].name.length));
                    if (sf != sv) {
                        $('#stat').text("New FW version is " + sf), $('#stat').css('color', 'red'),
                        $('#val').text("Your FW is out of date. Enable automatic update and reset IoTMeter."),
                        $('#myModal').modal('show');
                    }
...
$(function() {
    ...
    $(document).on("click", "#setSSID", function() {
        ...
        $(document).on("click", '#debug', function() {
            ...
            if (debug > 20) {
                $('.modal-body').text("Please reset wattmeter to switch testing FW")
                $('#myModal').modal('show');
                self.saveSetting("sw,TESTING SOFTWARE", 1);
            }
        });
});