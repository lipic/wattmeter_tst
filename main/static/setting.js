function Setting() {
    ...
    ((self = this).refreshSetting = function () {
                ...
                s.hasOwnProperty(e) &&
                    ...
                    $(
                        '<div class="row mt-4"><div class="col align-self-center"><p>' +
                            ...
                            e[0] +
                            e[1] +
                            '"type="text" class="add-color text-center height-25" maxlength="3" size="8" value="' +
                            s[e[0] + "," + e[1]] +
        ...
        (this.checkUpdate = function (t, s) {
            var e,
                n = parseFloat(t.substr(t.length - 5, t.length));
            (e = "1" == s ? "https://api.github.com/repos/lipic/wattmeter_tst/contents/" : "https://api.github.com/repos/lipic/wattmeter/contents/"),
                $.ajax({ url: e }).done(function (t) {
                    for (var s in t)
                        (t[s].name.includes("tst") || t[s].name.includes("rev")) &&
                            ((sf = parseFloat(t[s].name.substr(t[s].name.length - 5, t[s].name.length))),
                            sf != n && ($("#stat").text("New FW version is " + sf), $("#stat").css("color", "red"), $("#val").text("Your FW is out of date. Enable automatic update and reset IoTMeter."), $("#myModal").modal("show")));
                });
        }),
            ...
            for (var e = 1; e <= s; e++)
                $(
                    '<div id="evseSett"  class="container-sm pt-2 mt-3 text-center bg border border-secondary"><span class="dim">EVSE setting: ' +
                        e +
                        ' </span></div><div class="row mt-3"><div class="col align-self-center"><p>CURRENT [A]</p></div></div><div class="row"><div class="col"><div class="input-group" style="display: block; margin:auto;"><span class="input-group-btn"><button id="inp,EVSE' +
                        e +
        ...
        (this.modbusProccess = function (t, s, e, n) {
            $("#modbusStatus").text(""),
                "read" == e
                    ? ($("#modbusStatus").text("Reading register: " + s + " ..."), $("#readReg").append('<span class="spinner-border spinner-border-sm"></span>'), $("#modbusStatus").css("color", "#FBD428"))
                    : "write" == e &&
                      ($("#modbusStatus").text("Writing register: " + s + " with value: " + n + " ..."), $("#writeReg").append('<span class="spinner-border spinner-border-sm"></span>'), $("#modbusStatus").css("color", "#FBD428")),
                isNaN(s && t && n)
                    ...
                    : $.ajax({
                          ...
                          url: "/modbusRW",
                          async: !0,
                          data: JSON.stringify({ type: e, id: t, reg: s, value: n }),
                          success: function (t) {
                          ...
                          error: function () {
                              $("#modbusStatus").text("Response error"), $("#modbusStatus").css("color", "red"), $("#writeReg").find("span").remove(), $("#readReg").find("span").remove();
                          },
                      });
        });
}
        ...
        $(document).on("click", ".btnF", function () {
            ...
            (hodnota = 0),
                ...
                $(this).hasClass("btn-minuse") &&
                    ...
                    "inMAX-CURRENT-FROM-GRID-A" == t && (parseInt(s) - 1 < 0 ? ((hodnota = 0), $("#" + t).val(0)) : ((hodnota = parseInt(s) - 1), $("#" + t).val(hodnota))),
                ...
                $(this).hasClass("btn-plus") &&
                    ...
                    "inMAX-CURRENT-FROM-GRID-A" == t && (parseInt(s) + 1 > 125 ? ((hodnota = 125), $("#" + t).val(125)) : ((hodnota = parseInt(s) + 1), $("#" + t).val(hodnota))),
                    "inTIME-ZONE" == t && (parseInt(s) + 1 > 24 ? ((hodnota = 24), $("#" + t).val(24)) : ((hodnota = parseInt(s) + 1), $("#" + t).val(hodnota))),
