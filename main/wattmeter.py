...
class Wattmeter:
    ...
    async def wattmeterHandler(self):
        ...
        if((self.timeOfset == False)and(self.timeInit == True)):
            ...
            self.dataLayer.data['MounthlyEnergy'] = self.fileHandler.readData(self.MONTHLY_CONSUMPTION) 
            self.timeOfset = True
...
#TODO:
        if((self.lastMounth is not int(time.localtime()[1]))and self.timeInit and self.timeOfset):
           ...
           async with self.wattmeterInterface as w:
            ...
            self.dataLayer.data["MounthlyEnergy"] = self.fileHandler.readData(self.DAILY_CONSUMPTION) 
