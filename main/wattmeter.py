...
class Wattmeter:
    ...
    async def wattmeterHandler(self):
        ...
        if (self.timeOfset == False)and(self.timeInit == True):
            ...
            self.dataLayer.data["MonthlyEnergy"] = self.fileHandler.getMonthlyEnergy(self.DAILY_CONSUMPTION)
            self.timeOfset = True
        ...
        if (self.lastDay is not int(time.localtime()[2]))and self.timeInit and self.timeOfset:
            ...
            self.dataLayer.data["DailyEnergy"] =  self.fileHandler.readData(self.DAILY_CONSUMPTION,31)
    ...
    async def __readWattmeter_data(self,reg,length):
        ...
        try:
            ...
            if (receiveData != "Null") and (reg == 200):
                ...
                self.dataLayer.data["EminP"] = int(((receiveData[0]) << 8) | receiveData[1]) + int(((receiveData[2])<< 8)|receiveData[3]) + int((receiveData[4] << 8) |receiveData[5]) - int((receiveData[6] << 8) |receiveData[7]) - int((receiveData[8] << 8) |receiveData[9]) - int((receiveData[10] << 8) |receiveData[11])
                return "SUCCESS_READ"
...
class fileHandler:
    ...
    def readData(self,file,length=None):
        ...
        data = []
        try:
            f = (row.replace("\n","") for row in reversed(list(open(file))))
            for line in f:
                data.append(line)
                rows += 1
                if (length is not None) and (length <= rows):
                    data.reverse()
                    return data
                    #raise StopIteration
            data.reverse()
        ...
        try:
            ...
            for line in f:
                ...
                if len(rawData)<4:
                    continue
                if lastMonth == 0:
                    lastMonth = int(rawData[0])
                    lastYear = int(rawData[2])

                if lastMonth != int(rawData[0]):
                    monthCnt += 1
                ...
                positiveEnergy += int(rawData[3])
                negativeEnergy += int(rawData[4])
                    
            energy.append("{}/{}:[{},{}]".format(rawData[0],rawData[2],positiveEnergy,negativeEnergy))
            energy.reverse()
            return energy    
