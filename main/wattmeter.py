...
class Wattmeter:
    ...
    async def wattmeterHandler(self):
        ...
        if (self.lastDay is not int(time.localtime()[2]))and self.timeInit and self.timeOfset:
            ...
            day = {("{0:02}/{1:02}/{2}".format(time.localtime()[1],self.lastDay ,str(self.lastYear)[-2:] )) : [self.dataLayer.data["E1dP"] + self.dataLayer.data["E2dP"]+self.dataLayer.data["E3dP"], self.dataLayer.data["E1dN"] + self.dataLayer.data["E2dN"]+self.dataLayer.data["E3dN"]]}
            async with self.wattmeterInterface as w:
