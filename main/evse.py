...
class Evse():
    ...
    def balancingEvseCurrent(self):
        ...
        if (I1_P > I2_P)and(I1_P > I3_P):
            maxCurrent = int(round(I1_P/100.0))

        if (I2_P > I1_P)and(I2_P > I3_P):
            maxCurrent = int(round(I2_P/100.0))
            
        if (I3_P > I1_P)and(I3_P > I2_P):
            maxCurrent = int(round(I3_P/100.0))
        ...
        if delta < 0:
            if (self.__requestCurrent + delta)< 0:
                ...
                if (self.__requestCurrent + delta)< int(self.setting.config["in,MAX-CURRENT-FROM-GRID-A"])/2 :
