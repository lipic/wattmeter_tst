...
class Evse():
    ...
    def balancingEvseCurrent(self):
        ...
        if (I1 < I2)and(I1 < I3):
            minCurrent = int(round(I1/100.0))

        if (I2 < I1)and(I2 < I3):
            minCurrent = int(round(I2/100.0))
            
        if (I3 < I1)and(I3 < I2):
            minCurrent = int(round(I3/100.0))
            
        sumCurrent = I1 + I2 + I3
        avgCurrent = int(round(sumCurrent / 300))
        #print("Average current: ",avgCurrent)
        
        # "btn,SOLAR" 0-OFF, 1-1F, 2-3F
        #if self.dataLayer.data["AC_IN"] == 1:
        HDO = False
        if self.wattmeter.dataLayer.data["AC_IN"] and self.setting.config['sw,WHEN AC IN: CHARGING']:
            HDO = True
