...
class tcpModbus():
    ...
    async def proccessWattmeterData(self,fce,length,reg,receiveData,ID=1):
        ...
        if(fce == 16):
            return await self.wattmeter.writeWattmeterRegister(reg,tuple(int(receiveData[(2*i)] | receiveData[1+(2*i)]) for i  in range(0,length)))
    ...
    async def proccessEvseData(self,fce,length,reg,receiveData,ID=1):
        ...
        if(fce == 16):
            return await self.evse.writeEvseRegister(reg,tuple(int(receiveData[(2*i)] | receiveData[1+(2*i)]) for i  in range(0,length)),ID)
