...
class WebServerApp:
    ...
    def modbusRW(self,req, resp):
        ...
        if req.method == "POST":
            ...
            for i in req.form:
                ...
                if i['type'] == 'read':
                    try:
                        ...
                        if data is None:
                            self.datalayer = {"process":0,"value":"Error during reading register"}
                        else:
                            self.datalayer = {"process":1,"value":int(((data[0]) << 8) | (data[1]))}

                    except Exception as e:
                        print("Error during reading",e)
                        self.datalayer = {"process":e}
                    ...
                    try:
                        ...
                        if data is None:
                            self.datalayer = {"process":0,"value":"Error during writing register"}
                        else:
                            self.datalayer = {"process":1,"value":  int(((data[0]) << 8) | (data[1]))}
            ...
            yield from resp.awrite(json.dumps(self.datalayer))
            ...
            for i in req.form:
                ...
                if list(i.keys())[0] == 'relay':
                    if self.wattmeter.negotiationRelay():
                        self.datalayer = {"process":1}
                    else:
                        self.datalayer = {"process":0}
                    ...
                    self.datalayer = {"process":"OK"}
            yield from picoweb.jsonify(resp,self.datalayer)
                
        else:
            yield from picoweb.start_response(resp, "application/json")
            self.datalayer = self.wattmeter.dataLayer.data
            self.datalayer.update(self.evse.dataLayer.data)
            #picoweb.sendstream(resp,self.datalayer)
            #for i in self.datalayer:
            yield from resp.awrite(json.dumps(self.wattmeter.dataLayer.data))
    ...
    def updateWificlient(self,req, resp):
        ...
        if req.method == "POST":
            ...
            for i in req.form: 
                i = json.loads(i)
                self.datalayer = await self.wifiManager.handle_configure(i["ssid"],i["password"])
                self.ipAddress=self.wifiManager.getIp()
                self.datalayer = {"process":self.datalayer,"ip":self.ipAddress}
            yield from picoweb.start_response(resp, "application/json")
            yield from resp.awrite(json.dumps(self.datalayer))
    ...
    def updateSetting(self,req, resp):
        ...
        if req.method == "POST":
            ...
            for i in req.form: 
                i = json.loads(i)                    
                self.datalayer = setting.handle_configure(i["variable"],i["value"])
                self.datalayer = {"process":self.datalayer}
            
            yield from picoweb.start_response(resp, "application/json")
            yield from resp.awrite(json.dumps(self.datalayer))
                
        else:
            self.datalayer = setting.getConfig()
            yield from picoweb.start_response(resp, "application/json")
            yield from resp.awrite(json.dumps(self.datalayer))
    ...
    def getEspID(self,req,resp):
        setting = __config__.Config()
        self.datalayer = {"ID":" Wattmeter: {}".format(setting.getConfig()['ID']), "IP":self.wifiManager.getIp()}
        yield from picoweb.start_response(resp, "application/json")
        yield from resp.awrite(json.dumps(self.datalayer))
    ...
    async def webServerRun(self):
        try:
            ...
            threadName = self.app.run(debug=False, host='',port=self.port)
            while True:
